...
class combatHandler
{
    ...
    public function store(){
        ...
        $p1 = $this->p1->getId();
        $p2 = $this->p2->getId();
        $f1 = $this->p1->fortress->getId();
        $f2 = $this->f2->fortress->getId();
        $playerUp = $this->playerUp;
        $sql = "UPDATE gamehandler SET 
        basePoints = ?,
        p1 = ?,
        p2 = ?,
        f1 = ?,
        ...
        WHERE id = ?";
        $stmt = $this->mysqli->prepare($sql);
        $stmt->bind_param("isssss", $basePoints, $p1, $p2, $f1, $f2, $playerUp);
        $stmt->execute();
    }//End function store
    ...
    public function handleDamage($attacker, $defender, $rocket){
        ...
        $affinity = $this->handleAffinity($rocket->getType, $attacker->getCladding);
        if($affinity == false){
    ...
    public function handleAffinity($rocketType, $material){
        $affinity = NULL;
        if($rocket == 3 && $material == 1){
            $affinity = true;
        }
        if($rocket == 4 && $material == 2){
            $affinity = true;
        }
        if($rocket == 5 && $material == 3){
            $affinity = true;
        }
        if($rocket == 0 && $material == 3){
            $affinity = false;
        }
        if($rocket == 6 && $material == 3){
            $affinity = true;
        }
        if($rocket == 1 && $material == 5){
            $affinity = false;
        }
        if($rocket == 2 && $material == 6){
            $affinity = false;
        }
        if($rocket == 3 && $material == 7){
            $affinity = false;
        }
        if($rocket == 4 && $material == 8){
            $affinity = true;
        }
        return $affinity;
    }
    ...
    public function handleCombat($rocket){
        ...
        if($this->handleFlak($defender)){
            ...
            if($this->isHit($rocket->toHit, $defender->getHitResistance, $cladding)){
                //echo "it is a hit!<br/>";
...
public function findFortressInSlots($fId){
    if($this->f1->getId == $fId){
        return $this->f1;
    }elseif($this->f2->getId == $fId){
        return $this->f2;
    }
...
public function setUpFortresses($playerId){
    //Check new handler for empty fortress slots
           //If F1 is empty, create and store a new fortress
           //Otherwise, retrieve the fortress from DB and store it in F1
           ...
           if($this->F1 == NULL){
            ...
            $tempFortress = $this->f1->storeNew();
           }elseif($this->F1 != NULL){
                $tempFortress = $this->retrieveFortress($this->dbf1);
                $this->setF1($tempFortress);
                if($this->f1->getId = $row['fId']){
                    //F1 i2 player's fortress
                    $playerFortress = "f1";

                }
           } 
           //If F2 is empty, create and store a new fortress
           //Otherwise, retrieve the fortress from DB and store it in F2
           if($this->F2 == NULL){
            $tempFortress = $this->createFortress();
            $tempFortress->pId = $playerId;
            $tempFortress->setRandomName();
            ...
            if($this->f2->getId = $row['fId']){
                    //F2 is player's fortress
                    $playerFortress = "f2";
...
if(isset($_POST['username']) && isset($_POST['pass'])){
   echo $ch->authPlayer($_POST['username'], $_POST['pass']); 
...
if(isset($_POST['signup_username']) && isset($_POST['signup_pass'])){
    ...
    $this->player->setId();
    $this->player->setUsername = $row['username'];
    $this->player->setPass = $row['pass'];
    $this->player->store();
}
